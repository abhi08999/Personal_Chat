  ...
  return (
    ...
    >
      <div className={cn('relative max-w-[78%] sm:max-w-[68%]', isMine ? 'items-end' : 'items-start')}>
        <div
          onDoubleClick={() => !m.pending && !m.failed && onReact(m._id, '❤️')}
          ...
          className={cn(
            ...
            isMine
              ...
              : 'bg-gradient-bubble-them text-ink-900 border border-blush-200/60 rounded-bl-md'
          )}
        >
          ...
          {showReact && !m.pending && !m.failed && (
            <div className={cn(
              'absolute -top-9 flex gap-1 bg-white shadow-soft rounded-full px-2 py-1 border border-blush-200/60',
              isMine ? 'right-0' : 'left-0'
            )}>
              {['❤️','😂','😍','🥺','🔥','👍'].map((e) => (
                <button key={e} onClick={() => onReact(m._id, e)} className="hover:scale-125 transition text-base leading-none">
                  {e}
                </button>
              ))}
            </div>
        ...
        {(peerReaction || myReaction) && (
          <div className={cn('flex gap-1 mt-1', isMine ? 'justify-end' : 'justify-start')}>
            {myReaction && <span className="bg-white/80 border border-blush-200/60 rounded-full px-1.5 py-0.5 text-xs shadow-bubble">{myReaction}</span>}
            {peerReaction && peerReaction !== myReaction && <span className="bg-white/80 border border-blush-200/60 rounded-full px-1.5 py-0.5 text-xs shadow-bubble">{peerReaction}</span>}
          </div>
        )}

        <div className={cn('mt-1 flex items-center gap-1 text-[10px] text-ink-700/50', isMine ? 'justify-end' : 'justify-start')}>
          <span>{time}</span>
          {isMine && (
